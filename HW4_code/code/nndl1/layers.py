...
def affine_backward(dout, cache):
  ...
  # Copy and paste your code from HW #3
  dx_vector = dout.dot(w.T)
  dx = dx_vector.reshape(*x.shape)  
  
  x_vector = x.reshape(x.shape[0], w.shape[0])
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    x_hat = (x - sample_mean[:, np.newaxis].T)/ np.sqrt((sample_var[:, np.newaxis].T + eps))
    ...
    x_normalized = (x - running_mean[:, np.newaxis].T)/np.sqrt((running_var[:, np.newaxis].T + eps))
    # calculate out
...
def batchnorm_backward(dout, cache):
  ...
  db = (x - mu)*dx_hat
  de = -(1/2)*1/np.power(var+eps, 1.5)*db
  dsigma2 = np.sum(de, axis=0)
  dmu = -(1/np.sqrt(var + eps))*np.sum(dx_hat, axis=0) - dsigma2*(2/N)*np.sum(x-mu[:,np.newaxis].T,axis=0)
  dx = da + (2.0*(x-mu)/N)*dsigma2 + (1.0/N)*dmu
