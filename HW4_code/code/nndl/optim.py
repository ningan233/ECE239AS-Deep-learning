...
def sgd_nesterov_momentum(w, dw, config=None):
  ...
  # ================================================================ #
  v_old = v
  v_new = config['momentum'] * v_old - config['learning_rate'] * dw
  next_w = w + v_new + config['momentum'] * (v_new - v_old)
...
def rmsprop(w, dw, config=None):
  ...
  a = decay_rate * a + (1 - decay_rate) * dw * dw
  next_w = w - (learning_rate/(np.sqrt(a) + eps)) * dw
...
def adam(w, dw, config=None):
  ...
  v_mod = (1.0 / (1-np.power(beta1, t))) * v
  a_mod = (1.0 / (1-np.power(beta2, t))) * a
