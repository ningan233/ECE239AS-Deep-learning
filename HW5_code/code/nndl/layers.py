...
def affine_forward(x, w, b):
  ...
  # ================================================================ #
    ...
    dimension = w.shape[0]
    x_reshape = np.reshape(x, (num_inputs, dimension))
...
def affine_backward(dout, cache):
  ...
  # ================================================================ #
    ...
    db = dout.T.dot(np.ones(num_inputs))
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  """
    ...
    # ================================================================ #
        ...
        curBatch_mean = np.mean(x, axis = 0)
        curBatch_var = np.var(x, axis = 0)
       
        running_mean = momentum * running_mean + (1 - momentum) * curBatch_mean
        running_var = momentum * running_var + (1 - momentum) * curBatch_var
        bn_param['running_mean'] = running_mean
        bn_param['running_var'] = running_var
                    
        curBatch_norm = (x - curBatch_mean) / np.sqrt(curBatch_var + eps)
        out = gamma * curBatch_norm + beta
        
        cache = {
            'x_minus_mean':(x - curBatch_mean),
            'curBatch_norm': curBatch_norm,
            'gamma': gamma,
            'inv_var': 1.0 / np.sqrt(curBatch_var + eps),
            'sqrt_var': np.sqrt(curBatch_var + eps),
        }
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
    ...
    curBatch_norm = cache.get('curBatch_norm')
    gamma = cache.get('gamma')
    inv_var = cache.get('inv_var')
    sqrt_var = cache.get('sqrt_var')
    
    #dx
    dxhat = dout * gamma
    dxmu1 = dxhat * inv_var
    dinv_var = np.sum(dxhat * x_minus_mean, axis = 0)
    dsqrt_var = dinv_var * (-1.0 / sqrt_var**2)
    ...
    dgamma = np.sum(dout * curBatch_norm, axis = 0)
...
def dropout_forward(x, dropout_param):
  ...
  """
    ...
    # ================================================================ #
        ...
        dropout_factor = 1/(1-p)
        mask = mask * dropout_factor
