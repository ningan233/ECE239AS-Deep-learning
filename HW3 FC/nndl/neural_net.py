...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #X -> H1=W1X^t+b1 -> ReLU -> H2=W2X^t+b2 -> scores/softmax(loss)
    b1 = np.array([b1])
    b2 = np.array([b2])
    H1 = np.dot(W1,X.T) + b1.T #FC 1
    H1[H1<0] = 0 #ReLU
    H2 = np.dot(W2, H1) + b2.T #FC 2
    ...
    # scores is num_examples N by num_classes C
    max_scores = np.max(scores, axis = 1) #1xN
    max_scores = np.array([max_scores]).T #turn to Nx1
    scores -= max_scores
    class_probs = np.exp(scores)/np.sum(np.exp(scores), axis=1, keepdims=True) #NxC
    L_i = -np.log(class_probs[np.arange(N), y])
    L = np.sum(L_i)/N
    
    # 0.5 * reg * (||w||_F)^2
    loss = L + 0.5*reg*np.sum(W1**2) + 0.5*reg*np.sum(W2**2)
    ...
    db2 = np.sum(dscores, axis = 1)
    dW2 = np.dot(dscores, H1.T)
    temp = np.dot(W2.T, dscores)
    temp[H1<=0] = 0
    db1 = np.sum(temp, axis = 1)
    dW1 = np.dot(temp, X)
